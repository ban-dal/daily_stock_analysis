...
def normalize_analysis_context_pack_language(report_language: str = "zh") -> str:
    normalized = str(report_language or "").strip().lower().replace("_", "-")
    if normalized == "en":
        return "en"
    if normalized in {"ko", "kr", "ko-kr", "korean"}:
        return "ko"
    return "zh"
...
def _format_ko(payload: Dict[str, Any]) -> str:
    ...
    warnings = _list_strings(_nested(payload, "data_quality", "warnings"))
    if warnings:
        lines.append(f"- 데이터 품질 참고: {_join_text(warnings, lang='ko')}")
...
def _metadata_lines(payload: Dict[str, Any], *, lang: str) -> List[str]:
    ...
    return [
        {
            "zh": f"- 新闻结果数：{news_count}",
            "en": f"- News result count: {news_count}",
            "ko": f"- 뉴스 결과 수: {news_count}",
        }[lang]
    ]


def _data_limitation_lines(payload: Dict[str, Any], *, lang: str) -> List[str]:
    lines = ["", {"zh": "## 数据限制", "en": "## Data Limitations", "ko": "## 데이터 제한"}[lang]]
    ...
    if limitations:
        label = {"zh": "已知限制", "en": "Known limitations", "ko": "알려진 제한"}[lang]
        separator = "： " if lang == "ko" else (": " if lang == "en" else "：")
    ...
    if _has_core_degraded_block(payload):
        if lang == "en":
            ...
            lines.append(
                "- 신뢰도 규칙: quote, daily_bars 또는 technical 데이터가 stale, fallback, "
                "missing, fetch_failed, partial 또는 estimated이면 최종 JSON confidence_level은 High가 될 수 없습니다."
            )
        else:
            lines.append(
    ...
    if lang == "en":
        ...
        lines.append(
            "- 분석 규칙: 보조 데이터 블록 누락은 해당 분석 섹션만 제한하며, 누락 자체를 상승 또는 하락 신호로 해석하지 마세요."
        )
        lines.append(
            "- 안전 규칙: 이 요약의 status, source, warnings, missing_reason만 사용하고 raw payload, 뉴스 본문, 원시 추세값, secret, token, webhook은 재현하지 마세요."
        )
    else:
        lines.append(
...
def _phase_data_quality_constraint_lines(payload: Dict[str, Any], *, lang: str) -> List[str]:
    ...
    if lang == "ko":
        if phase in INTRADAY_MARKET_PHASES:
            return [
                "- 단계/데이터 규칙: 장중 판단은 실시간 시세, 일봉 또는 기술 데이터 품질의 제한을 받으므로 단기 매매 결론 전에 해당 제한을 명시하세요."
            ]
        if phase == "premarket":
            return [
                "- 단계/데이터 규칙: 장전 계획은 데이터 신선도 또는 폴백 상태의 제한을 받으므로 저하된 시세 데이터를 오늘 완료된 가격 흐름처럼 설명하지 마세요."
            ]
        if phase in CONSERVATIVE_MARKET_PHASES:
            return [
                "- 단계/데이터 규칙: 현재 사용 가능한 데이터만 보수적으로 사용하고 존재하지 않는 장중 사실을 보완하지 마세요."
            ]
        return []
...
def _quality_level_label(level: str, *, lang: str) -> str:
    labels = {
        "zh": QUALITY_LEVEL_LABELS_ZH,
        "en": QUALITY_LEVEL_LABELS_EN,
        "ko": QUALITY_LEVEL_LABELS_KO,
    }[lang]
